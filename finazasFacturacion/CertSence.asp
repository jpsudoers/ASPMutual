...
'	theTable.NextRow	
	...
	dim datosProg
datosProg="select E.ID_EMPRESA,E.RUT,UPPER(E.R_SOCIAL) as R_SOCIAL,C.DESCRIPCION,C.CODIGO,CONVERT(VARCHAR(10),P.FECHA_INICIO_, 105) as FECHA_INICIO,"
...
set rsMutual = conn.execute ("SELECT RUT,R_SOCIAL,CONTACTO,RUT_CONTACTO FROM INSTITUCION")
	...
	theTable.NextCell
	theDoc.HPos = 0
	theTable.AddText rsDatosProg("R_SOCIAL")
	theTable.SelectCell(1)
	theTable.Frame True, True, true, true
	...
	theTable.AddText replace(FormatNumber(mid(rsDatosProg("RUT"), 1,len(rsDatosProg("RUT"))-2),0)&mid(rsDatosProg("RUT"), len(rsDatosProg("RUT"))-1,len(rsDatosProg("RUT"))),",",".")
	theTable.SelectCell(1)
	theTable.Frame True, True, true, true
	...
	N_FACT="SELECT FACTURA FROM FACTURAS where ID_AUTORIZACION='"&IdAuto&"' and ID_EMPRESA='"&rsDatosProg("ID_EMPRESA")&"'"
	
	set RSFACT = conn.execute (N_FACT)

	theTable.NextCell
	theDoc.HPos = 0
	
	if not RSFACT.eof and not RSFACT.bof then 
		theTable.AddText RSFACT("FACTURA")
	else
		theTable.AddText ""	
	end if
