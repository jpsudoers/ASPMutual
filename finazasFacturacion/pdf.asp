...
if(Request("relator")<>"0")then
   relator=" and HISTORICO_CURSOS.RELATOR="&Request("relator")
end if

v_empresa=""
if(Request("empresa")<>"")then
   v_empresa=" and HISTORICO_CURSOS.ID_EMPRESA in ("&Request("empresa")&")"
end if

v_autorizacion=""
if(Request("autorizacion")<>"")then
   v_autorizacion=" and HISTORICO_CURSOS.ID_AUTORIZACION="&Request("autorizacion")
end if

dim tot
tot="select COUNT(distinct ID_EMPRESA) as total from HISTORICO_CURSOS "
tot=tot&" where HISTORICO_CURSOS.ID_PROGRAMA="&Request("prog")&relator&v_empresa
'tot=tot&" and HISTORICO_CURSOS.RELATOR="&Request("relator")

set Total = conn.execute (tot)

dim trabEmpQuery
trabEmpQuery="select distinct ID_EMPRESA from HISTORICO_CURSOS where HISTORICO_CURSOS.ID_PROGRAMA="&Request("prog")&relator&v_empresa

set rsEmp = conn.execute (trabEmpQuery)
'" and HISTORICO_CURSOS.RELATOR="&Request("relator"))
...
function Tabla(theDoc)
	...
	set rgSence = conn.execute ("select ISNULL(convert(varchar,a.N_REG_SENCE),'') as N_REG_SENCE from AUTORIZACION a where a.ID_AUTORIZACION='"&Request("autorizacion")&"'")
	
	theTable.NextRow
	theTable.NextCell
	theDoc.HPos = 0	
	theTable.AddText "N° REGISTRO SENCE"
	theTable.NextCell
	theDoc.HPos = 0
	theTable.AddText rgSence("N_REG_SENCE")
...
function Tabla2(theDoc)
	...
	sql2 = sql2&" CURRICULO.HORAS as horas,EMPRESAS.RUT as rut, UPPER(EMPRESAS.R_SOCIAL) as r_social, "
	...
	sql2 = sql2&"(CASE WHEN SEDES.ID_SEDE = 27 THEN bloque_programacion.nom_sede "
	sql2 = sql2&" WHEN SEDES.ID_SEDE <>  27 THEN SEDES.DIRECCION+' '+SEDES.CIUDAD END) as direccion "
	...
	sql2 = sql2&" inner join bloque_programacion on bloque_programacion.id_bloque=HISTORICO_CURSOS.ID_BLOQUE " 	
	sql2 = sql2&" inner join SEDES on SEDES.ID_SEDE=HISTORICO_CURSOS.SEDE "
	sql2 = sql2&" where HISTORICO_CURSOS.ID_PROGRAMA="&Request("prog")&" and HISTORICO_CURSOS.ID_EMPRESA="&empresa_id&relator
...
function TablaParticipantes(theDoc)
	...
	sql3 = sql3&" where HISTORICO_CURSOS.ID_PROGRAMA="&Request("prog")&" and HISTORICO_CURSOS.ID_EMPRESA="&empresa_id&relator&v_autorizacion
