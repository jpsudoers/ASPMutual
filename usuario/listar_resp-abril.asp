...
sql = "SELECT id_usuario, rut, NOMBRES+' '+A_PATERNO+' '+A_MATERNO as nombre_usuario from USUARIOS WHERE ESTADO=1 ORDER BY rut asc, "&columna&" "&orden
'response.Write(sql)
'response.End()
...
WHILE NOT DATOS.EOF
	if(fila>=inicio AND fila<(limite*pagina))then
		...
		Response.Write("<cell><![CDATA["&DATOS("nombre_usuario")&"]]></cell>"&chr(13))
		Response.Write("<cell><![CDATA[<span class=""ui-state-valid"" ><a href=""#"" title=""Modificar Registro"" class=""ui-icon ui-icon-pencil"" name=""aContrato"" onclick=""update("&DATOS("ID_USUARIO")&")""></a></span>]]></cell>"&chr(13))
Response.Write("<cell><![CDATA[<span class=""ui-state-valid"" ><a href=""#"" title=""Eliminar Registro"" class=""ui-icon ui-icon-trash"" name=""aContrato"" onclick=""eliminar("&DATOS("ID_USUARIO")&")""></a></span>]]></cell>"&chr(13))
