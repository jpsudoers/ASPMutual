...
query= query&" where HISTORICO_CURSOS.ID_BLOQUE="&vid

set rsEmp = conn.execute (query)

if not rsEmp.eof and not rsEmp.bof then 
%>
 <form name="frmcierreeval" id="frmcierreeval" action="evaluacioncierre/modificarIngresoEval.asp" method="post" ">
 <table cellspacing="0" cellpadding="1" border=0>
     ...
     <tr>
      ...
      <table id="mytable" border="1" width="960" ONMOUSEOVER= 'cambiaEstado();'>
      <thead> 
      <tr>
           <th>Rut / Ident.</th> 
           <th>Alumno</th> 
      ...
      <tbody> 
           <%
			   ...
			   sol = sol&"CURRICULO.PORCE_ASISTENCIA, CURRICULO.PORCE_CALIFICACION, EVA_CDN=ISNULL(HISTORICO_CURSOS.EVA_CDN,'Reprobado'),"
			   sol = sol&"EMPRESAS.RUT as emp_rut,dbo.MayMinTexto(EMPRESAS.R_SOCIAL) as R_SOCIAL, HISTORICO_CURSOS.PO,HISTORICO_CURSOS.PVMO from HISTORICO_CURSOS "
			   ...
			   sol = sol&" where HISTORICO_CURSOS.ID_BLOQUE="&vid
          ...
          <tr>
           ...
           <td><select name="po<%=i%>" id="po<%=i%>">
             <option value="0" <%if rsSol("PO")=0 Then%> selected="selected" <%End if%>>No</option>
             <option value="1" <%if rsSol("PO")=1 Then%> selected="selected" <%End if%>>Si</option>
           </select></td>
           <td><select name="pvmo<%=i%>" id="pvmo<%=i%>">
           <option value="0"<%if rsSol("PVMO")=1 Then%> selected="selected" <%End if%>>No</option>
             <option value="1"<%if rsSol("PVMO")=1 Then%> selected="selected" <%End if%>>Si</option>
           </select></td>
           <td><label id="<%=Eva&i%>" name="<%=Eva&i%>"><%=rsSol("EVA_CDN")%></label><input id="<%=Eva2&i%>" name="<%=Eva2&i%>" type="hidden" value="<%=mid(rsSol("EVA_CDN"),1,1)%>"/></td>
          </tr>
          <%
		  	   rsSol.Movenext
			wend
      ...
      <table cellspacing="0" cellpadding="0" border=0>
          <tr>
           <td><input id="countFilas" name="countFilas" type="hidden" value="<%=i%>"/><input id="u" name="u" type="hidden" value="<%=Request("u")%>"/><input id="b" name="b" type="hidden" value="<%=Request("b")%>"/><input id="t" name="t" type="hidden" value="<%=Request("t")%>"/><input id="ec2" name="ec2" type="hidden" value="<%=Request("ec")%>"/><input id="i" name="i" type="hidden" value="<%=Request("id")%>"/><input id="r" name="r" type="hidden" value="<%=Request("relator")%>"/></td>
          </tr>
       </table>
</form> 
<%
