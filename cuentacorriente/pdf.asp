...
function Tabla(theDoc)
	...
	n=0
        ...
        fac = fac&" from FACTURAS FC where FC.ESTADO=1 AND FC.ID_EMPRESA="&rs("empresa_id")
		...
		totSaldoParcial=0
			...
			n_pagos=1
					if(n=1)then
					    ...
					    totMontosFac = cdbl(cdbl(totMontosFac) + cdbl(rsFac("VALOR_OC")))
					   ...
					   PagCount=PagCount&" I.id_factura="&rsFac("ID_FACTURA")&" and E.ID_EMPRESA="&rs("empresa_id")
						...
						totSaldoParcial=0
								while not rsPag.eof
									...
									totPagosFac = cdbl(cdbl(totPagosFac) + cdbl(totPag))
		...
		rs.Movenext
	wend
	totSaldo = cdbl(cdbl(totMontosFac) - cdbl(totPagosFac))
	theTable.SelectRow theTable.Row
