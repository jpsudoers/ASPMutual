...
'sql = "select EMPRESAS.RUT,EMPRESAS.R_SOCIAL,AUTORIZACION.ORDEN_COMPRA,AUTORIZACION.FACTURA,"
'sql = sql&"AUTORIZACION.VALOR_OC,AUTORIZACION.ID_AUTORIZACION from AUTORIZACION "
'sql = sql&" inner join EMPRESAS on EMPRESAS.ID_EMPRESA=AUTORIZACION.ID_EMPRESA "
'sql = sql&" WHERE AUTORIZACION.FACTURA IS not NULL "
'sql = sql&" ORDER BY rut asc, "&columna&" "&orden

sql = "select EMPRESAS.RUT as rut,EMPRESAS.R_SOCIAL as empresa, "
sql = sql&"AUTORIZACION.ORDEN_COMPRA as orden, " 
sql = sql&"AUTORIZACION.VALOR_OC as valor, AUTORIZACION.ID_AUTORIZACION as ID_AUTORIZACION " 
sql = sql&" from AUTORIZACION "
sql = sql&" inner join EMPRESAS on EMPRESAS.ID_EMPRESA=AUTORIZACION.ID_EMPRESA " 
sql = sql&" where AUTORIZACION.FACTURA is null "
sql = sql&" order by AUTORIZACION.ID_AUTORIZACION desc "
...
WHILE NOT DATOS.EOF
	if(fila>=inicio AND fila<(limite*pagina))then
		...
		Response.Write("<cell><![CDATA["&"$ "&replace(FormatNumber(DATOS("valor"),0),",",".")&"]]></cell>"&chr(13))
