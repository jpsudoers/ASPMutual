...
sql = "SELECT ID_INSTRUCTOR, rut, dbo.MayMinTexto(NOMBRES+' '+A_PATERNO+' '+A_MATERNO) as nombre_instructor from INSTRUCTOR_RELATOR WHERE ESTADO =1 ORDER BY "&columna&" "&orden
...
WHILE NOT DATOS.EOF
	if(fila>=inicio AND fila<(limite*pagina))then
		...
		Response.Write("<cell><![CDATA["&DATOS("nombre_instructor")&"]]></cell>"&chr(13))
		Response.Write("<cell><![CDATA[<span class=""ui-state-valid"" ><a href=""#"" title=""Modificar Registro"" class=""ui-icon ui-icon-pencil"" name=""aContrato"" onclick=""update("&DATOS("ID_INSTRUCTOR")&")""></a></span>]]></cell>"&chr(13))
Response.Write("<cell><![CDATA[<span class=""ui-state-valid"" ><a href=""#"" title=""Eliminar Registro"" class=""ui-icon ui-icon-trash"" name=""aContrato"" onclick=""eliminar("&DATOS("ID_INSTRUCTOR")&")""></a></span>]]></cell>"&chr(13))
