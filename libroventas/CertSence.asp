...
set rsMutual = conn.execute ("SELECT RUT,R_SOCIAL,CONTACTO,RUT_CONTACTO FROM INSTITUCION")
	...
	theTable.NextCell
	theDoc.HPos = 0
	theTable.AddText ucase(request("nomEmpresa"))
	theTable.SelectCell(1)
	theTable.Frame True, True, true, true
	...
	theTable.AddText replace(FormatNumber(mid(request("rutEmpresa"), 1,len(request("rutEmpresa"))-2),0)&mid(request("rutEmpresa"), len(request("rutEmpresa"))-1,len(request("rutEmpresa"))),",",".")
	theTable.SelectCell(1)
	theTable.Frame True, True, true, true
	...
	theTable.NextCell
	theDoc.HPos = 0
	theTable.AddText ucase(request("nomOtic"))
	theTable.SelectCell(1)
	theTable.Frame True, True, true, true
	...
	theDoc.HPos = 0
	'theTable.AddText rsDatosProg("RUT")
	if(request("rutOtic")<>"No Aplica")then
		theTable.AddText replace(FormatNumber(mid(request("rutOtic"), 1,len(request("rutOtic"))-2),0)&mid(request("rutOtic"), len(request("rutOtic"))-1,len(request("rutOtic"))),",",".")
	else
		theTable.AddText ucase(request("rutOtic"))
	end if
	theTable.SelectCell(1)
	theTable.Frame True, True, true, true
	...
	theTable.NextCell
	theDoc.HPos = 0
	theTable.AddText request("nFactura")
	theTable.SelectCell(1)
	theTable.Frame True, True, true, true	
	...
	theTable.NextCell
	theDoc.HPos = 0
	theTable.AddText ""&rsDatosProg("N_REG_SENCE")
	theTable.SelectCell(1)
	theTable.Frame True, True, true, true		
	...
	wend	
		...
		set rsRL = conn.execute ("select CONVERT(VARCHAR(10),f.FECHA_EMISION, 105) as FECHA from facturas f where factura="&request("nFactura"))
		
		'if(cdbl(year(cdate(rsRL("FECHA"))))<=2013)then 
