...
sql = "select PROGRAMA.ID_PROGRAMA,CURRICULO.CODIGO,CURRICULO.SENCE as SENCE,PROGRAMA.CUPOS,"
sql = sql&"dbo.MayMinTexto (CURRICULO.NOMBRE_CURSO) as NOMBRE_CURSO,CONVERT(VARCHAR(10),PROGRAMA.FECHA_INICIO_, 105) as FECHA_INICIO_,"
sql = sql&"(select COUNT(*) from HISTORICO_CURSOS where HISTORICO_CURSOS.ID_PROGRAMA=PROGRAMA.ID_PROGRAMA) as inscritos, "
sql = sql&"PROGRAMA.TIPO, case when PROGRAMA.ID_EMPRESA is not null then "
sql = sql&" ' - '+(select dbo.MayMinTexto (E.R_SOCIAL) from EMPRESAS E where "
sql = sql&"E.ID_EMPRESA=PROGRAMA.ID_EMPRESA)+' ('+PROGRAMA.DIR_EJEC+')' else (case when PROGRAMA.DIR_EJEC is not null then "
sql = sql&"' - '+PROGRAMA.DIR_EJEC else '' end) end as R_SOCIAL "
...
WHILE NOT DATOS.EOF
	if(fila>=inicio AND fila<(limite*pagina))then
		...
		if(DATOS("TIPO")="2")then 
			...
			Response.Write("<cell><![CDATA["&DATOS("NOMBRE_CURSO")&DATOS("R_SOCIAL")&"]]></cell>"&chr(13))
			Response.Write("<cell><![CDATA["&DATOS("CUPOS")&"]]></cell>"&chr(13))
			Response.Write("<cell><![CDATA["&DATOS("inscritos")&"]]></cell>"&chr(13))
		else
			Response.Write("<cell><![CDATA[<b>"&DATOS("FECHA_INICIO_")&"</b>]]></cell>"&chr(13))		
			Response.Write("<cell><![CDATA[<b>"&right("000000000"&DATOS("ID_PROGRAMA"),5)&"</b>]]></cell>"&chr(13))
			Response.Write("<cell><![CDATA[<b>"&DATOS("CODIGO")&"</b>]]></cell>"&chr(13))
			Response.Write("<cell><![CDATA[<b>"&DATOS("NOMBRE_CURSO")&DATOS("R_SOCIAL")&"</b>]]></cell>"&chr(13))
			Response.Write("<cell><![CDATA[<b>"&DATOS("CUPOS")&"</b>]]></cell>"&chr(13))
			Response.Write("<cell><![CDATA[<b>"&DATOS("inscritos")&"</b>]]></cell>"&chr(13))
		end if
