...
cabeceraPagina.SetFile Server.MapPath("../images/fondoCertCodelcoCursos2020.jpg")
...
if(Request("empresa")<>"0")then
	...
	histTrab=histTrab&" H.COD_AUTENFICACION,T.NACIONALIDAD,'CURSO'=dbo.MayMinTexto(C.NOMBRE_CURSO) from HISTORICO_CURSOS H "
	histTrab=histTrab&" inner join PROGRAMA p on p.ID_PROGRAMA=h.ID_PROGRAMA "
	histTrab=histTrab&" INNER JOIN currICULO C ON C.ID_MUTUAL=p.ID_MUTUAL"
...
function Tabla(theDoc)
	...
	theDoc.Font = theDoc.EmbedFont("Cambria")
		...
		theTable.NextCell
		theDoc.HPos = 0.5	
	        'theTable.AddText rsHistTrab("CURSO")
		'theTable.AddHtml("<b>"&rsHistTrab("CURSO")&"</b>")
		theDoc.AddHtml "<b>"&rsHistTrab("CURSO")&"</b>"
	

		theTable.NextRow
		theDoc.Font = theDoc.EmbedFont("Cambria")
		theDoc.Rect = "160 2 790 92"
	...
	if(rsHistTrab("RutRel")<>"11616799-9")then
	theTable.NextRow
	FirmaRel.SetFile Server.MapPath("../images/FR-"&rsHistTrab("RutRel")&".jpg")		
	theDoc.Rect = "400 177 580 105"
