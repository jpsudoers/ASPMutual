...
function Tabla2(theDoc)
	...
	'sql3 = sql3&" and HISTORICO_CURSOS.	="&Request("relator")
    ...
    sql3 = sql3&" WHEN bq.id_sede <>  27 THEN s.NOMBRE+', '+s.DIRECCION+', '+s.COMUNA END) as sede,"
    ...
    sql3 = sql3&" WHEN bq.id_rel_seg IS NULL THEN '' END) as rel_seg,c.ID_MUTUAL "
 	...
 	sql3 = sql3&" where bq.id_relator='"&Request("relator")&"' and bq.id_programa='"&Request("prog")&"' and bq.id_bloque='"&Request("idbloque")&"'"
	
	
	set rs3 = conn.execute (sql3)
	
	idMutual=rs3("ID_MUTUAL")
	f_dia1=rs3("FECHA_INICIO_")
	f_dia2=rs3("FECHA_TERMINO")
	'/******* CABECERA *********/
	theTable.Frame True, False, False, False
	theDoc.FrameRect
	...
	theDoc.HPos = 0
	'if(idMutual<>"40")then
	if(idMutual<>"40" and idMutual<>"41")then
...
'if(idMutual="40")then
if(idMutual="40" or idMutual="41")then
theDoc.FontSize = 12
	...
	theTable.Width(0) = 0.6
	theTable.Width(1) = 2.2
	theTable.Width(2) = 1.3
	theTable.Width(3) = 0.9
	theTable.Width(4) = 0.9
	theTable.Width(5) = 0.9
	theTable.Width(6) = 0.9
	theTable.Width(7) = 1
	theTable.Width(8) = 0.9	
	theTable.Padding = 5
	...
	sql3 = sql3&" and HISTORICO_CURSOS.id_bloque="&Request("idbloque")	
	...
	while not rs3.eof
		...
		theTable.SelectCell(1)
		theTable.Frame True, True, True, True
		If fila Mod 2 = 1 Then theTable.Fill "220 220 220"
		theTable.NextCell
		theDoc.HPos = 1
		theTable.AddText ""
		...
		if(fila=10 or fila=20 or fila=30 or fila=40 or fila=50)then
...
'DANIELA	
	...
	theTable.Width(8) = 1
	theTable.Width(9) = 0.9	
	theTable.Width(10) = 0.9			
	theTable.Padding = 5
	...
	theDoc.HPos = 0
	theTable.AddText ""
	theTable.SelectCell(9)
	theTable.Frame True, false, false, false
		theTable.Fill "174 174 174"		
	theTable.NextCell
	theDoc.HPos = 0
	theTable.AddText ""
	theTable.SelectCell(10)
	theTable.Frame True, false, false, true
		theTable.Fill "174 174 174"			
...
function Tabla6(theDoc)
	...
	theTable.Padding = 6
	    ...
	    rsBq="select CB.ID_BLOQUE_CURSO,CB.NOMBRE_BLOQUE,CB.HORAS,CB.DIA from CURRICULO_BLOQUE CB WHERE CB.ESTADO=1 AND CB.ID_MUTUAL="&idMutual			
		
		set rsBloque = conn.execute (rsBq)		
		
		cambiaFecha=0
		while not rsBloque.eof
		rsAcvd="SELECT CA.TEMA,CA.ACTIVIDAD FROM CURRICULO_ACTIVIDADES CA WHERE CA.ESTADO=1 and CA.ID_BLOQUE_CURSO="&rsBloque("ID_BLOQUE_CURSO")	

		set rsActividad = conn.execute (rsAcvd)	
		contbloque=0
			   
			   while not rsActividad.eof
			        contbloque=contbloque+1
			   		theTable.NextRow
					theTable.NextCell
					theDoc.HPos = 0.5
					if(contbloque=1)then
						if(rsBloque("DIA")="1" and cambiaFecha=0)then
							theTable.AddText f_dia1
							cambiaFecha=1
							theTable.SelectCell(0)
					        theTable.Frame True, False, true, False
						elseif(rsBloque("DIA")="2" and cambiaFecha=1)then
							theTable.AddText f_dia2
							cambiaFecha=2
							theTable.SelectCell(0)
					        theTable.Frame True, False, true, False
						elseif(rsBloque("DIA")="3" and cambiaFecha=2)then
							theTable.AddText f_dia2
							cambiaFecha=3
							theTable.SelectCell(0)
					        theTable.Frame True, False, true, False							
					...
					theTable.NextCell
					theDoc.HPos = 0
					theTable.AddText ""&rsActividad("TEMA")
					theTable.SelectCell(1)
					theTable.Frame False, False, true, False
					theTable.NextCell
					theDoc.HPos = 0
					theTable.AddText ""&rsActividad("ACTIVIDAD")
					theTable.SelectCell(2)
					theTable.Frame False, False, true, False
					theTable.NextCell
					theDoc.HPos = 0.5
					if(contbloque=2)then
					    theTable.AddText rsBloque("HORAS")
					else
						theTable.AddText ""
					end if
					theTable.SelectCell(3)
					...
					if(contbloque=1)then
						theDoc.Font = theDoc.EmbedFont("Arial Black")
						theTable.AddText rsBloque("NOMBRE_BLOQUE")
					elseif(contbloque=2)then
						    theTable.AddText rsBloque("HORAS")
					else
						theTable.AddText ""
					end if			
...
function Tabla7(theDoc)
	...
	theTable.Width(0) = 0.6
	theTable.Width(1) = 2.4
	theTable.Width(2) = 1.5
	theTable.Width(3) = 1.6
	theTable.Width(4) = 1.6
	theTable.Width(5) = 1.6
	theTable.Width(6) = 2.1
	theTable.Width(7) = 0.6
	...
	while not rs3.eof
		...
		theTable.SelectCell(0)
		theTable.Frame True, True, True, True
        If participante Mod 2 = 1 Then theTable.Fill "220 220 220"
		theTable.NextCell
		theDoc.HPos = 0
		theTable.AddText rs3("NOMBRES")
		theTable.SelectCell(1)
        ...
        'If i = UBound(participante) Then 
		'theTable.Frame True, True, False, False
		'theTable.SelectRow theTable.Row
		'theTable.Frame True, True, False, True
        'If participante Mod 2 = 1 Then theTable.Fill "220 220 220"	
		
		if(participante=10 or participante=20 or participante=30 or participante=40 or participante=50)then
			theDoc.Rect = "30 50 760 570"
		    theDoc.Page = theDoc.AddPage()
			theDoc.FontSize = 12
	...
	For cuen = participante+1 To 30 Step 1
		...
		if(participante=10 or participante=20)then
			theDoc.Rect = "30 50 760 570"
		    theDoc.Page = theDoc.AddPage()
			theDoc.FontSize = 12
	...
	'if(idMutual="39" or idMutual="40")then
	if(idMutual="39" or idMutual="40" or idMutual="41")then
		TablaHipo(theDoc)
		'theDoc.Rect = "30 50 760 760"
	end if
end function

function TablaHipo(theDoc)
	theDoc.FontSize = 12
	...
	theDoc.AddHtml ("<b>CAMPAÑA DE DIFUSIÓN SOBRE PREVENCIÓN DE RIESGOS Y RELACIONADOS A LA EXPOSICIÓN LABORAL A GRAN ALTURA E HIPOBARIA.</b>")
	theDoc.Font = theDoc.EmbedFont("Arial")
...
function TablaMeno(theDoc)
	Set theTable = New Table
	theTable.Focus theDoc, 1
	theTable.Width(0) = 8

	sql3="select p.ID_PROGRAMA,c.CODIGO,c.NOMBRE_CURSO,c.DESCRIPCION,ir.NOMBRES+' '+ir.A_PATERNO+' '+ir.A_MATERNO as instructor,"
 	sql3 = sql3&"ir.rut,CONVERT(VARCHAR(10),p.FECHA_INICIO_, 105) as FECHA_INICIO_,"
	sql3 = sql3&"CONVERT(VARCHAR(10),p.FECHA_TERMINO, 105) as FECHA_TERMINO,"
 	sql3 = sql3&"(CASE WHEN bq.id_sede =  27 THEN bq.nom_sede "
    sql3 = sql3&" WHEN bq.id_sede <>  27 THEN s.NOMBRE END) as sede,"
    sql3 = sql3&"(CASE WHEN bq.id_rel_seg IS NOT NULL THEN ' / '+(select ri.rut "
	sql3 = sql3&" from INSTRUCTOR_RELATOR ri where ri.ID_INSTRUCTOR=bq.id_rel_seg) "
    sql3 = sql3&" WHEN bq.id_rel_seg IS NULL THEN '' END) as rut_rel_seg,"	
    sql3 = sql3&"(CASE WHEN bq.id_rel_seg IS NOT NULL THEN ' / '+(select ri.NOMBRES+' '+ri.A_PATERNO+' '+ri.A_MATERNO "
	sql3 = sql3&" from INSTRUCTOR_RELATOR ri where ri.ID_INSTRUCTOR=bq.id_rel_seg) "
    sql3 = sql3&" WHEN bq.id_rel_seg IS NULL THEN '' END) as rel_seg,c.ID_MUTUAL "
 	sql3 = sql3&"from bloque_programacion bq "
	...
	theTable.NextCell
	theDoc.HPos = 0
	theDoc.Rect = "30 560 190 600"
	theDoc.AddImageObject theImg, False		
	
	theTable.NextRow
	theTable.NextCell
	theDoc.HPos = 0.5
	theTable.AddText "MEMORANDO REPROBACIÓN"
	...
	theTable.NextCell
	theDoc.HPos = 0	
	theTable.AddText "NOMBRE DEL RELATOR : "&UCase(rs3("instructor"))&UCase(rs3("rel_seg"))
	theTable.SelectCell(0)
	theTable.Frame True, True, True, True	
	...
	theTable.NextCell
	theDoc.HPos = 0	
	theTable.AddText "RUT DEL RELATOR : "&UCase(rs3("rut"))&UCase(rs3("rut_rel_seg"))
	theTable.SelectCell(0)
	theTable.Frame True, True, True, True	
	...
	theTable.NextCell
	theDoc.HPos = 0	
	theTable.AddText "LUGAR DE REALIZACIÓN : "&UCase(rs3("sede"))
	theTable.SelectCell(0)
	theTable.Frame True, True, True, True	
	...
	theTable.NextCell
	theDoc.HPos = 0	
	theTable.AddText "CURSO : "&UCase(rs3("DESCRIPCION"))
	theTable.SelectCell(0)
	theTable.Frame True, True, True, True	
	...
	theTable.NextCell
	theDoc.HPos = 0	
	theTable.AddText "FECHA DEL CURSO : FECHA INICIO "&rs3("FECHA_INICIO_")&" - FECHA TÉRMINO "&rs3("FECHA_TERMINO")
	theTable.SelectCell(0)
	theTable.Frame True, True, True, True	
	...
	Set theTable = New Table
	theTable.Focus theDoc, 3
	theTable.Width(0) = 2.3
	theTable.Width(1) = 1.3
	theTable.Width(2) = 4.4
	...
	theTable.NextCell
	theDoc.HPos = 0.5
	theTable.AddText "RUT PARTICIPANTE"
	theTable.SelectCell(1)
	theTable.Frame False, False, False, False
		'theTable.Fill "174 174 174"
	theTable.NextCell
	theDoc.HPos = 0.5
	theTable.AddText "MOTIVO DE REPROBACIÓN"
	theTable.SelectCell(2)
	theTable.Frame False, False, False, False
		'theTable.Fill "174 174 174"
	...
	theTable.Padding = 8
	participante=0
	For cuen = participante+1 To 9 Step 1	
		participante=participante+1
		theTable.NextRow
		theTable.NextCell
		theDoc.HPos = 0.5
	...
	theTable.NextCell
	theDoc.HPos = 0.5
	theTable.AddText "Firma del relator"
	theTable.SelectCell(0)
	theTable.Frame True, false, false, false	
...
function TablaMeno2(theDoc)
	...
	theTable.NextCell
	theDoc.HPos = 0.5
	theTable.AddText "OBSERVACIONES DEL RELATOR"
	...
	theTable.NextCell
	theDoc.HPos = 0	
	theTable.AddText "NOMBRE DEL CURSO : "&UCase(rs3("DESCRIPCION"))
	theTable.SelectCell(0)
	theTable.Frame True, True, True, True	
	...
	theTable.Padding = 8
	participante=0
	For cuen = participante+1 To 10 Step 1	
		participante=participante+1
		theTable.NextRow
		theTable.NextCell
		theDoc.HPos = 0.5
