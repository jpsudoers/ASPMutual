...
sql2 = sql2&" where HISTORICO_CURSOS.ID_PROGRAMA=PROGRAMA.ID_PROGRAMA)) > 0 AND TIPO=1 "
...
WHILE NOT DATOS_CURSOS.EOF
		if(fila>=inicio AND fila<(limite*pagina))then
			...
			Response.Write("<cell><![CDATA[<span class=""ui-state-valid"" ><a href=""#"" title=""Modificar Registro"" class=""ui-icon ui-icon-pencil"" name=""aContrato"" onclick=""update("&DATOS_CURSOS("ID_PROGRAMA")&","&DATOS_CURSOS("ins_vacantes")&")""></a></span>]]></cell>"&chr(13))
			Response.Write("</row>"&chr(13)) 
		end if
	fila=fila+1
