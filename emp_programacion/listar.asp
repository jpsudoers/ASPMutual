...
sql = sql&"(CASE WHEN PROGRAMA.DIR_EJEC <>  'NULL' THEN ' - '+PROGRAMA.DIR_EJEC else '' END) as direccion,"
sql = sql&" Modalidad=(case when PROGRAMA.ID_Modalidad=1 then 'Presencial' "
sql = sql&" when PROGRAMA.ID_Modalidad=2  then 'Asincrónico' "
sql = sql&" when PROGRAMA.ID_Modalidad=0 then 'Virtual' else '' end) "
...
sql2 = "select PROGRAMA.ID_PROGRAMA,CURRICULO.CODIGO,(CASE WHEN CURRICULO.SENCE = 0 THEN 'Si' "
...
sql2 = sql2&"(CASE WHEN PROGRAMA.DIR_EJEC <>  'NULL' THEN ' - '+PROGRAMA.DIR_EJEC else '' END) as direccion,"
sql2 = sql2&" Modalidad=(case when PROGRAMA.ID_Modalidad=1 then 'Presencial' "
sql2 = sql2&" when PROGRAMA.ID_Modalidad=2  then 'Asincrónico' "
sql2 = sql2&" when PROGRAMA.ID_Modalidad=0 then 'Virtual' else '' end) "
sql2 = sql2&" from PROGRAMA "
...
sql2 = sql2&" where HISTORICO_CURSOS.ID_PROGRAMA=PROGRAMA.ID_PROGRAMA)) > 0 AND TIPO=1 and id_empresa='"&Session("usuario")&"'"
...
WHILE NOT DATOS.EOF
		if(fila>=inicio AND fila<(limite*pagina))then
			...
			Response.Write("<cell><![CDATA["&DATOS("Modalidad")&"]]></cell>"&chr(13))
			...
			Response.Write("<cell><![CDATA[<span class=""ui-state-valid"" ><a href=""#"" title=""Modificar Registro"" class=""ui-icon ui-icon-pencil"" name=""aContrato"" onclick=""update("&DATOS("ID_PROGRAMA")&","&DATOS("ins_vacantes")&",2)""></a></span>]]></cell>"&chr(13))
			else
...
WHILE NOT DATOS_CURSOS.EOF
		if(fila>=inicio AND fila<(limite*pagina))then
			...
			Response.Write("<cell><![CDATA[<b>"&DATOS_CURSOS("Modalidad")&"</b>]]></cell>"&chr(13))
			Response.Write("<cell><![CDATA[<b>"&DATOS_CURSOS("NOMBRE_CURSO")&DATOS_CURSOS("direccion")&"</b>]]></cell>"&chr(13))
			...
			if(Session("activa_empresa")="1")then
			Response.Write("<cell><![CDATA[<span class=""ui-state-valid"" ><a href=""#"" title=""Modificar Registro"" class=""ui-icon ui-icon-pencil"" name=""aContrato"" onclick=""update("&DATOS_CURSOS("ID_PROGRAMA")&","&DATOS_CURSOS("ins_vacantes")&",1)""></a></span>]]></cell>"&chr(13))
			else
				if(Session("activa_empresa")="0")then
					Response.Write("<cell><![CDATA[<span class=""ui-state-valid"" ><a href=""#"" title=""Modificar Registro"" class=""ui-icon ui-icon-pencil"" name=""aContrato"" onclick=""bloqueo();""></a></span>]]></cell>"&chr(13))
				else
