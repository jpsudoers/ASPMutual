...
Response.AddHeader "content-disposition", "inline; filename=Historico_Cursos_"&fecha&".xls"
	...
	'if(request("id_empresa")<>"")then
		IDemp=" and HISTORICO_CURSOS.ID_EMPRESA=2841"
	'end if
	
	'IDmutual=""
	'if(request("id_mutual")<>"0")then
		IDmutual=" and PROGRAMA.ID_MUTUAL in (2108,2106,2110,2109,2111,2107)"
	'end if	
	...
	qsHc=qsHc&" END) as 'Tipo Documento',AUTORIZACION.ORDEN_COMPRA,'http://norte.otecmutual.cl/ordenes/'+ AUTORIZACION.DOCUMENTO_COMPROMISO as DOCUMENTO_COMPROMISO,AUTORIZACION.ID_PROYECTO,"
	qsHc=qsHc&"CF=(CASE WHEN AUTORIZACION.CON_FRANQUICIA=1 THEN 'SI' ELSE 'NO' END),trabajador.cargo_empresa,"
	qsHc=qsHc&"CONVERT(VARCHAR(10),AUTORIZACION.FECHA_REV_CIERRE, 105) as FECHA_BAJA,AUTORIZACION.N_REG_SENCE, isnull(HISTORICO_CURSOS.PO,0) as PO, isnull(HISTORICO_CURSOS.PVMO,0) as PVMO, trabajador.NSAP, trabajador.GERENCIA from HISTORICO_CURSOS " 
