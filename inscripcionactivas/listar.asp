...
if(Request("doc")<>"T")then
  if(Request("doc")="SC" OR Request("doc")="C")then
		TipoDoc=" and (CASE WHEN AUTORIZACION.CON_OTIC=1 AND AUTORIZACION.CON_FRANQUICIA=1 AND AUTORIZACION.TIPO_DOC=4 "
		TipoDoc=TipoDoc&" AND (AUTORIZACION.N_REG_SENCE IS NULL OR AUTORIZACION.N_REG_SENCE IS NOT NULL) then '0' " 
		TipoDoc=TipoDoc&"  WHEN AUTORIZACION.CON_OTIC=0 AND AUTORIZACION.CON_FRANQUICIA=1 AND AUTORIZACION.TIPO_DOC=4 AND AUTORIZACION.N_REG_SENCE IS NOT NULL then '0' " 
		TipoDoc=TipoDoc&"  WHEN AUTORIZACION.CON_OTIC=0 AND AUTORIZACION.CON_FRANQUICIA=0 AND AUTORIZACION.TIPO_DOC=4 then '0' "
		TipoDoc=TipoDoc&"  WHEN AUTORIZACION.CON_OTIC=1 AND AUTORIZACION.CON_FRANQUICIA=1 AND AUTORIZACION.TIPO_DOC<>4 AND AUTORIZACION.N_REG_SENCE IS NULL then '0' " 
		TipoDoc=TipoDoc&"  WHEN AUTORIZACION.CON_OTIC=0 AND AUTORIZACION.CON_FRANQUICIA=1 AND AUTORIZACION.TIPO_DOC<>4 then '1' " 
		TipoDoc=TipoDoc&"  WHEN AUTORIZACION.CON_OTIC=0 AND AUTORIZACION.CON_FRANQUICIA=0 AND AUTORIZACION.TIPO_DOC<>4 then '1' "  
		TipoDoc=TipoDoc&"  WHEN AUTORIZACION.CON_OTIC=1 AND AUTORIZACION.CON_FRANQUICIA=1 AND AUTORIZACION.TIPO_DOC<>4 AND AUTORIZACION.N_REG_SENCE IS NOT NULL then '1' END)="
	if(Request("doc")="SC")then
  	...
  	TipoDoc=TipoDoc&" and (select COUNT(*) from verifica_inscripcion VI where VI.ID_AUTORIZACION=AUTORIZACION.ID_AUTORIZACION AND VI.ESTADO=1)>0"	
  end if
end if
...
sql = sql&" (select COUNT(*) from HISTORICO_CURSOS hc where hc.ID_AUTORIZACION=AUTORIZACION.ID_AUTORIZACION and hc.ESTADO=1) as evaluado "
...
sql = sql&" WHERE AUTORIZACION.ESTADO=1 and AUTORIZACION.ID_PROGRAMA in (select p2.ID_PROGRAMA from PROGRAMA p2 "
sql = sql&" where CONVERT(date,p2.FECHA_INICIO_)>=CONVERT(date,'18-08-2011')) "&prog&emp&TipoDoc&" ORDER BY "&columna&" "&orden

'sql = sql&" and AUTORIZACION.ID_PROGRAMA in (select p2.ID_PROGRAMA from PROGRAMA p2 where CONVERT(date,p2.FECHA_INICIO_)>CONVERT(date,'18-08-2011'))"
...
WHILE NOT DATOS.EOF
	if(fila>=inicio AND fila<(limite*pagina))then
		...
		if(DATOS("evaluado")="0")then
		Response.Write("<cell><![CDATA[<span class=""ui-state-valid"" ><a href=""#"" title=""Ver Pre Certificado de Asistencia"" class=""ui-icon ui-icon-script"" name=""aContrato"" onclick=""verCertificado("&DATOS("ID_AUTORIZACION")&")""></a></span>]]></cell>"&chr(13))							
		else
		Response.Write("<cell><![CDATA[<span class=""ui-state-valid"" ><a href=""#"" title=""Inscripción de Curso No Evaluado"" class=""ui-icon ui-icon-cancel"" name=""aContrato""></a></span>]]></cell>"&chr(13))	
		end if			
		
		Response.Write("<cell><![CDATA[<span class=""ui-state-valid"" ><a href=""#"" title=""Unir Inscripciones"" class=""ui-icon ui-icon-newwin"" name=""aContrato"" onclick=""selUnirInsc("&DATOS("ID_AUTORIZACION")&")""></a></span>]]></cell>"&chr(13))

		Response.Write("<cell><![CDATA[<span class=""ui-state-valid"" ><a href=""#"" title=""Separar Inscripcion"" class=""ui-icon ui-icon-extlink"" name=""aContrato"" onclick=""selSepInsc("&DATOS("ID_AUTORIZACION")&")""></a></span>]]></cell>"&chr(13))
		...
		if(Request("doc")="R")then
			Response.Write("<cell><![CDATA[<span class=""ui-state-valid""><a href=""#"" title=""Rechazos Inscripciones"" class=""ui-icon ui-icon-arrowthickstop-1-s"" name=""aContrato"" onclick=""dgrzIns("&DATOS("ID_AUTORIZACION")&");""></a></span>]]></cell>"&chr(13))
		else
			Response.Write("<cell><![CDATA[<span class=""ui-state-valid""><a href=""#"" title=""Rechazos Inscripciones"" class=""ui-icon ui-icon-cancel"" name=""aContrato""></a></span>]]></cell>"&chr(13))
		end if		
