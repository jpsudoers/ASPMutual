...
vorden = Request("txtNOrden")
vvalortotal = Request("ValorTotal")
vnparticipantes = Request("NParticipantes")
vbloque = Request("txtBloque")
vsala = Request("txtSala")
vrelator = Request("txtRelator")
vempresa = Request("Empresa")
vprograma = Request("Programa")
vautorizacionid = Request("autorizacionId")
vdocactual = Request("DocActual")
vtabpart = Request("tabFechaPart")
vconotic = Request("ConOtic")
vcofran = Request("COfran")

if(Request("NReg_Fran")<>"")then
	vreg_fran = Request("NReg_Fran")
else
	vreg_fran = "NULL"
end if
...
dim UpAutoriza
UpAutoriza = "update AUTORIZACION set ID_PROGRAMA='"&vprograma&"', ORDEN_COMPRA='"&vorden&"', "
UpAutoriza = UpAutoriza&"VALOR_OC='"&vvalortotal&"', CON_OTIC='"&vconotic&"', " 
UpAutoriza = UpAutoriza&"CON_FRANQUICIA='"&vcofran&"', N_REG_SENCE="&vreg_fran&", "
UpAutoriza = UpAutoriza&"ID_BLOQUE='"&vbloque&"', TIPO_DOC='"&vcompromiso&"', N_PARTICIPANTES='"&vnparticipantes&"' "
UpAutoriza = UpAutoriza&" where ID_AUTORIZACION='"&vautorizacionid&"' and ID_EMPRESA='"&vempresa&"'"

conn.execute (UpAutoriza)

dim UpHistorico
UpHistorico = "update HISTORICO_CURSOS set ID_PROGRAMA='"&vprograma&"'," 
UpHistorico = UpHistorico&" RELATOR='"&vsala&"', SEDE='"&vrelator&"', ID_BLOQUE='"&vbloque&"' "
UpHistorico = UpHistorico&" where ID_AUTORIZACION='"&vautorizacionid&"' and ID_EMPRESA='"&vempresa&"'"
...
texto_fichero = Replace(texto_fichero,"<!--#doc_tipo-->","http://norte.otecmutual.cl/ordenes/"&vdocactual)
