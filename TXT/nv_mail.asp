...
set ref = archivo.OpenTextFile(server.mappath("TXTLink/TXT_NV_SFT_"&fecha&".txt"), 2, True)
	...
	qsHc=" SELECT CONVERT(VARCHAR(10),F.FECHA_EMISION,105) AS FECHA_FACTURA,"&_
	 ...
	 "DBO.MayMinTexto(E.COMUNA) AS COMUNA, DBO.MayMinTexto(E.CIUDAD) AS CIUDAD,NOC=dbmas.dbo.RemoveCharacteres(ORDEN_COMPRA),"&_
	...
	if rsHc("ES_OTIC")=1 then
		...
		DescripcionCCC="D.:"&rsHc("SEDE")&"^Codigo:"&rsHc("CODIGO")&"^N.H.:"&replace(cstr(rsHc("HORAS")),"-","/")&_
			...
			ref.writeline(rsHc("ID_FACTURA")&";"&replace(cstr(rsHc("FECHA_FACTURA")),"-","/")&";"&replace(cstr(rsHc("FECHA_FACTURA")),"-","/")&";;"&rsHc("NOC")&";"&mid(rsHc("RUT"), 1,len(rsHc("RUT"))-2)&";01;;;"&Descripcion&";;05;;"&rsHc("ID_CECO")&";;;;;;;;;;;;;;;;;;;;;;"&rsHc("COD_SOFTLAND")&";;1;"&rsHc("MONTO")&";;;;;;;;;;;;"&DescripcionCCC&";")
	...
	Set objFile = archivo.GetFile(server.mappath("TXTLink/TXT_NV_SFT_"&fecha&".txt"))
	...
	'objStream.LoadFromFile Server.MapPath("TXTLink/TXT_NV_SFT_"&fecha&".txt")
	...
	With iMsg
	.To = "cmgonzalez@mutualcapacitacion.cl; vaguirre@mutualasesorias.cl; sdurand@masesorias.cl"
