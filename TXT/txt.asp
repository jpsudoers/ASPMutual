...
set ref = archivo.OpenTextFile(server.mappath("TXT_SFT_"&fecha&".txt"), 2, True)
	...
	if rsHc("ES_OTIC")=1 then
		...
		Descripcion=rsHc("SEDE")&"#1"&rsHc("CODIGO")&"#2"&replace(cstr(rsHc("HORAS")),"-","/")&_
			...
			ref.writeline("22;"&rsHc("ID_FACTURA")&";"&replace(cstr(rsHc("FECHA_FACTURA")),"-","/")&";01;"&Descripcion&";"&mid(rsHc("RUT"), 1,len(rsHc("RUT"))-2)&";"&rsHc("ID_CECO")&";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"&rsHc("COD_SOFTLAND")&";"&rsHc("MONTO")&";;1;;;;;;;;;;;;;;;;;")
