...
set ref = archivo.OpenTextFile(server.mappath("TXT_NV_SFT_"&fecha&".txt"), 2, True)
	...
	qsHc=" SELECT CONVERT(VARCHAR(10),F.FECHA_EMISION,105) AS FECHA_FACTURA,"&_
	 ...
	 " RutBenef=(select o.RUT from EMPRESAS o where o.ID_EMPRESA=a.ID_EMPRESA),U.ID_USUARIO_NETSUITE,'PAGO'=(CASE WHEN a.TIPO_DOC='0' then 'Pago a 30 días' else 'Al Contado' END) "&_
	 " FROM FACTURAS F "&_
