...
vid = request("id")
if(request("id")="undefined") then
	vid=0
end if
sql = "select ID_EMPRESAS_USUARIOS, ID_EMPRESA, NOMBRE, CARGO, TELEFONO, EMAIL"
sql = sql&" from EMPRESAS_USUARIOS "
sql = sql&" where ID_EMPRESA='"&vid&"' and ESTADO=1"
...
WHILE NOT RespuestaListado.EOF
	if(fila>=inicio AND fila<(limite*pagina))then
		...
		Response.Write("<cell><![CDATA[<span class=""ui-state-valid"" ><a href=""#"" title=""Modificar Registro"" class=""ui-icon ui-icon-pencil"" name=""aContrato"" onclick=""update2("&RespuestaListado("ID_EMPRESAS_USUARIOS")&")""></a></span>]]></cell>"&chr(13))
