...
qsMan="select PROGRAMA.ID_PROGRAMA,CURRICULO.CODIGO,PROGRAMA.FECHA_INICIO_,"&_
	  ...
	  " (select ISNULL(Z.nombre,'') from bloque_programacion bq   "&_
	  " inner join sedes s on s.ID_SEDE=bq.id_sede  "&_
	  " INNER JOIN ZONA Z ON Z.id_zona = S.ID_ZONA "&_
	  " where bq.id_programa=PROGRAMA.ID_PROGRAMA) as ZONA,  "&_	  
	  " (select COUNT(*) from HISTORICO_CURSOS where HISTORICO_CURSOS.ID_PROGRAMA=PROGRAMA.ID_PROGRAMA "&_
	  ...
	  " where PROGRAMA.ESTADO=1 AND PROGRAMA.ID_CLIENTE in (11)  "&proSel&mesSel&anoSel&_
    ...
    <table width="800" border="0">
        ...
        <tr>
	      ...
	      <td style="border-width: 1px;border: solid; border-color:#000;"><font size="2"><%=rsMan("ZONA")%></font></td>	          
