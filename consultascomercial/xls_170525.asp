...
qsMan="select PROGRAMA.ID_PROGRAMA ,CURRICULO.CODIGO ,PROGRAMA.FECHA_INICIO_, dbo.MayMinTexto (CURRICULO.NOMBRE_CURSO) as NOMBRE_CURSO ,CONVERT(VARCHAR(10),PROGRAMA.FECHA_INICIO_, 105) as FECHA_INICIO ,CONVERT(VARCHAR(10),PROGRAMA.FECHA_TERMINO, 105) as FECHA_TERMINO ,PROGRAMA.CUPOS, (select COUNT(*) from HISTORICO_CURSOS where HISTORICO_CURSOS.ID_PROGRAMA=PROGRAMA.ID_PROGRAMA) as inscritos, (select COUNT(*) from HISTORICO_CURSOS where HISTORICO_CURSOS.ID_PROGRAMA=PROGRAMA.ID_PROGRAMA and HISTORICO_CURSOS.ASISTENCIA>0 and HISTORICO_CURSOS.ESTADO=0) as Asistentes, (select COUNT(*) from HISTORICO_CURSOS where HISTORICO_CURSOS.ID_PROGRAMA=PROGRAMA.ID_PROGRAMA and HISTORICO_CURSOS.EVALUACION='Aprobado' and HISTORICO_CURSOS.ESTADO=0) as Aprobados, (select ISNULL(Z.nombre,'')  from bloque_programacion bq Inner join sedes s on s.ID_SEDE=bq.id_sede INNER JOIN ZONA Z ON Z.id_zona = S.ID_ZONA  where  bq.id_programa=PROGRAMA.ID_PROGRAMA) AS  ZONAS, (select COUNT(*) from HISTORICO_CURSOS where HISTORICO_CURSOS.ID_PROGRAMA=PROGRAMA.ID_PROGRAMA and HISTORICO_CURSOS.EVALUACION='Reprobado' and HISTORICO_CURSOS.ESTADO=0) as Reprobados, PROGRAMA.TIPO AS TIPO ,(case when PROGRAMA.DIR_EJEC is not null then PROGRAMA.DIR_EJEC else '' end) as SEDE,(select case when bq.id_sede<>27 then s.NOMBRE+', '+s.DIRECCION+', '+s.CIUDAD when bq.id_sede=27 then bq.nom_sede end from bloque_programacion bq  inner join sedes s on s.ID_SEDE=bq.id_sede where bq.id_programa=PROGRAMA.ID_PROGRAMA) as sede_curso ,(select case when bq.id_sede<>27 then ISNULL((select cm.NOMBRE from comuna cm where cm.ID_COMUNA=s.ID_COMUNA),'') when bq.id_sede=27 then ISNULL((select cm.NOMBRE from comuna cm where cm.ID_COMUNA=bq.id_comuna),'') end from bloque_programacion bq inner join sedes s on s.ID_SEDE=bq.id_sede where bq.id_programa=PROGRAMA.ID_PROGRAMA) as 'Comuna' ,(select case when bq.id_sede<>27 then ISNULL((select r.NOMBRE from regiones r where r.ID_REGION=s.ID_REGION),'') when bq.id_sede=27 then ISNULL((select r.NOMBRE from regiones r where r.ID_REGION=bq.id_region),'') end from bloque_programacion bq inner join sedes s on s.ID_SEDE=bq.id_sede where bq.id_programa=PROGRAMA.ID_PROGRAMA) as 'Region' ,(select case when bq.id_sede<>27 then ISNULL((select r.id_region from regiones r where r.ID_REGION=s.ID_REGION),'') when bq.id_sede=27 then ISNULL((select r.id_region from regiones r where r.ID_REGION=bq.id_region),'') end from bloque_programacion bq inner join sedes s on s.ID_SEDE=bq.id_sede where bq.id_programa=PROGRAMA.ID_PROGRAMA) as 'idRegion' ,(select s.ID_COD_INT_MUTUAL from bloque_programacion bq inner join sedes s on s.ID_SEDE=bq.id_sede where bq.id_programa=PROGRAMA.ID_PROGRAMA) as id_sede_curso ,HI=(select hb.HORARIO from HORARIO_BLOQUES hb where hb.ID_HORARIO=PROGRAMA.BMI) ,HF=(select hb.HORARIO from HORARIO_BLOQUES hb where hb.ID_HORARIO=PROGRAMA.BTF) ,IR.RUT as Rut_instructor ,IR.NOMBRES+' '+Ir.A_PATERNO+' '+Ir.A_MATERNO AS instructor ,CURRICULO.ID_PROYECTO ,(CASE CURRICULO.ID_PROYECTO WHEN 1 THEN 'Abierta' WHEN 2 THEN 'Cerrada' WHEN 3 THEN 'Proyecto Almagro' WHEN 4 THEN 'Spot Mutual' WHEN 5 THEN 'Cerrado-Collahuasi' ELSE '' end) as 'PROYECTO' ,(select dbo.Estado_evaluacion(bq.estado_eva_cdn) from bloque_programacion bq where bq.id_programa=PROGRAMA.ID_PROGRAMA) as 'EC' ,  usr=ISNULL((select u.NOMBRES+' '+u.A_PATERNO+' '+u.A_MATERNO from usuarios u where u.ID_USUARIO=PROGRAMA.USUARIO_CREA),'') , PROGRAMA.ESTADO ,CURRICULO.PGP ,VW.* from PROGRAMA inner join CURRICULO on CURRICULO.ID_MUTUAL=PROGRAMA.ID_MUTUAL LEFT JOIN bloque_programacion BQ ON BQ.id_programa = PROGRAMA.ID_PROGRAMA LEFT JOIN INSTRUCTOR_RELATOR IR ON ir.ID_INSTRUCTOR=bq.id_relator LEFT JOIN VW_PIVOT_GASTO_PROGRAMA VW ON VW.ID_BLOQUE = BQ.id_bloque where PROGRAMA.ESTADO=1 AND PROGRAMA.ID_CLIENTE in (11)  "&proSel&mesSel&anoSel& "order by programa.FECHA_INICIO_"
